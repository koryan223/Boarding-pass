-- Update get_student_analytics to explicitly use Monday as week start
...
BEGIN
    ...
    -- Calculate current week start on Monday (PostgreSQL date_trunc('week') uses Monday by default per ISO 8601)
    current_week_start := date_trunc('week', CURRENT_TIMESTAMP);
    
    -- Get successful swipes this week (Monday onwards)
    ...
    -- Get re-entries this week (Monday onwards)
    ...
    -- Get weekly data for the month with Monday start (date_trunc('week') uses Monday by default)
    ...
    -- Calculate average swipes per week with Monday start
